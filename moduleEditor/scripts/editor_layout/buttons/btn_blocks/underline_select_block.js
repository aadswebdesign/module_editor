//buttons/btn_blocks/underline_select_block.js
import * as BEE from './../btn_elems_export.js';
export async function underlineSelectBlock(...args){
	const [icon_option]= args;
	const icon = icon_option ? icon_option : 'underline-select-icon';
	const elem_data = {};
	elem_data.block_btn = {};
	elem_data.block_btn.elem_id = 'u_select_block';
	elem_data.block_btn.elem_classes = ['select','on-off','btn-block','relative'];
	const parent_elem = BEE.btnBlockElem(elem_data.block_btn);
	elem_data.btn = {};
	elem_data.btn.elem_id = 'u_select_btn';
	elem_data.btn.elem_classes = ['tb-item','tb-style',icon,'relative'];
	elem_data.btn.elem_title = 'Underline Select';

//scripts/editor_logic/actions/handlers/insert_hx_el_to_art_ftr.js
import * as MFT from './../../../factory/module_functions.js';
import * as MDFT from './../../../factory/module_dom_functions.js';
class InsertHxElToArtFtr{
	#br_el;
	#created_elem;
	#ex_parents;
	#ex_prevs;
	...
	constructor(...args){
		...
		(async()=> {
			if(this.#parent_el.lastElementChild !== null){
				...
				if(this.present_parent.lastElementChild !== null && this.present_parent.lastElementChild.tagName === parent_tag_name){
					this.ftr_parent = this.present_parent.lastElementChild;
					//console.log('ftr_parent: ',this.ftr_parent);
					this.insert_elems(this.ftr_parent);
				}
		...
		//console.table({'InsertHxElToArtFtr': args});
	}
	insert_elems = function(...args){
		const [parent_el] = args;
		(async()=> {
			let first_child;
			//console.log('ftr parent_el: ',parent_el);
...
export const insertHxElToArtFtr = async (...args)=>{
	return new InsertHxElToArtFtr(...args);
}
